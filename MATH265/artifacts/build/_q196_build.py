...
Pattern from Q17.x-Q19.5 (carryover 27.12, 28.14, 29.12, 30.12):
  ...
  - fresh classes, every invented name carrying the 196 suffix, checked against
...
SRC = os.path.join(ART, "Q19.5-which-factor-varies.html")
OUT = os.path.join(ART, "Q19.6-the-equal-area-rectangle.html")

sys.path.insert(0, HERE)
import _q196_fig as F  # noqa: E402
...
assert F.congruent_control(), "the congruence guard's control did not run"
...
# --------------------------------------------------------------- 4. page CSS
FIG_CSS = """
/* ---- Q19.6 figure classes.  Emitted BEFORE the body, in this same block. */
.plotbg196{fill:var(--plot)}
/* an area fill takes its opacity off the SOLID token.  The -soft tokens are
   10 to 13 percent alpha and vanish against the plot ground - 30.8 shipped a
   figure whose one idea was invisible for exactly that reason. */
.fill196{fill:var(--fam);fill-opacity:.20;stroke:none}
.rectbox196{fill:none;stroke:var(--accent);stroke-width:1.8}
.wedgeA196{fill:var(--num);fill-opacity:.30;stroke:var(--num);stroke-width:1}
.wedgeB196{fill:var(--num);fill-opacity:.30;stroke:var(--num);stroke-width:1}
.tline196{fill:none;stroke:var(--fam);stroke-width:2.4;stroke-linecap:round}
.avg196{stroke:var(--accent);stroke-width:1.3;stroke-dasharray:2 3}
.cline196{stroke:var(--ink3);stroke-width:1.2;stroke-dasharray:3 3}
.cdot196{fill:var(--accent)}
.panel196{fill:none;stroke:var(--line);stroke-width:1}
.curve196{fill:none;stroke:var(--fam);stroke-width:2.2}
.chord196{stroke:var(--ink3);stroke-width:1.2;stroke-dasharray:4 3}
.eplev196{stroke:var(--num);stroke-width:2.6}
.avglev196{stroke:var(--accent);stroke-width:1.4;stroke-dasharray:3 3}
/* ONE colour mapping across the whole page, settled by looking:
   ...
   amber --accent = the object being constructed, here the average and the
                    rectangle that defines it
   slate --num    = a NEUTRAL region or level that is neither right nor wrong:
                    figure 1's two leftover wedges are the ARGUMENT, not an
                    error, and figure 2's endpoint level is correct in one
                    panel and wrong in two, so neither may borrow the rust
   Panel headers and the neutral comparison columns stay unpainted, because a
   comparison with no right and wrong side must not borrow the pair (30.9). */
.tick196{fill:var(--plotlab);font:500 10px var(--mono);letter-spacing:.04em}
.lab{fill:var(--plotlab);font:500 11px var(--mono);letter-spacing:.05em}
.tlab196{fill:var(--fam)}
.arealab196{fill:var(--fam)}
.avgl196{fill:var(--accent)}
.clab196{fill:var(--accent)}
.wedgeAl196{fill:var(--num)}
.wedgeBl196{fill:var(--num)}
.panl196{fill:var(--ink)}
.same196{fill:var(--accent)}
.hi196{fill:var(--los)}
.lo196{fill:var(--los)}
...
EXTRA = """
/* ---- Q19.6 page classes.  Every invented name carries the 196 suffix.
...
.lede196{font:500 clamp(15px,1.6vw,17.5px)/1.62 var(--sans);color:var(--ink2);
  max-width:64ch;margin:14px 0 0}
.ansbar196{display:grid;gap:1px;background:var(--line);border:1px solid var(--line);
  grid-template-columns:repeat(auto-fit,minmax(min(232px,100%),1fr))}
.ac196{background:var(--surface2);padding:16px 18px;display:flex;
  flex-direction:column;gap:8px}
.al196{font:600 10px var(--mono);letter-spacing:.13em;color:var(--accent)}
.av196{font:600 18px/1.4 var(--sans);color:var(--ink)}
.small196{font:400 13.5px/1.58 var(--sans);color:var(--ink2)}
.steps196{display:grid;gap:1px;background:var(--line);border:1px solid var(--line);
  margin:18px 0}
.sp196{background:var(--surface);padding:13px 16px;display:grid;
  grid-template-columns:30px minmax(0,1fr);gap:12px;align-items:baseline}
.sp196 > span:nth-child(2){grid-column:2}
.sn196{grid-column:1;font:600 12px var(--mono);color:var(--accent);letter-spacing:.06em}
.box196{border-left:3px solid var(--accent);background:var(--sunk);
  padding:6px 20px;margin:20px 0;overflow-x:auto}
.note196,.warn196{border-left:3px solid var(--fam);background:var(--sunk);
  padding:14px 18px;margin:18px 0;font:400 14.5px/1.62 var(--sans);color:var(--ink2)}
.warn196{border-left-color:var(--los)}
.tscroll{overflow-x:auto;margin:16px 0;border:1px solid var(--line)}
.tab196{width:100%;min-width:860px;border-collapse:collapse;
  font:400 13.5px/1.5 var(--sans)}
.tab196 th{font:600 10px var(--mono);letter-spacing:.11em;color:var(--ink3);
  text-align:left;padding:9px 12px;border-bottom:1px solid var(--line)}
.tab196 td{padding:9px 12px;border-bottom:1px solid var(--line-soft);
  vertical-align:top}
.tab196 tbody tr:last-child td{border-bottom:none}
.num196{font-variant-numeric:tabular-nums;font-family:var(--mono);
...
.tab196 td.ok196,.ok196{color:var(--fam)}
.tab196 td.no196,.no196{color:var(--los)}
.tab196 tr.hot196 td{background:var(--los-soft)}
.tab196 tr.truth196 td{background:var(--accent-soft)}
.tree196{font:500 12px/1.62 var(--mono);color:var(--ink2);background:var(--sunk);
...
.tab196 td mjx-container,.tab196 th mjx-container{color:inherit}
.tab196 td mjx-container svg{fill:currentColor}
@media (max-width:760px){.tab196{min-width:700px}}
"""
...
# ------------------------------------------------------------------ 6. body
body = open(os.path.join(HERE, "_q196_body.html"), encoding="utf-8").read()
