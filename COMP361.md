...
* **Analogy:** A cooking recipe. You wash the vegetables, *then* you chop them, *then* you sauté them. Each step is an action.
...
The noun technique is a more formal, methodical approach. Instead of relying on a meeting, the analyst combs through every piece of documentation—interview transcripts, use case descriptions, and existing forms—to find every noun.
...
### Pro-Tip
A common mistake is confusing an **Attribute** with a **Class**. If a "thing" has its own properties (like a *Customer* having a name, address, and phone number), it’s a **Class**. If a "thing" is just a single piece of information (like *Color* or *Zip Code*), it’s almost always an **Attribute**.
...
> **Note:** Sometimes, you’ll see an identifier marked with a specific symbol (like a **#** or the word **{PK}**) in a diagram to show it’s the "key" to the record.
...
* **Example:** `Address` is the classic compound attribute. It isn't just one long string; it’s composed of `streetAddress`, `city`, `state`, and `zipCode`. 
...
* If the answer is **Yes**, use **Aggregation**. (e.g., A Library and its Books—the books still exist if the library closes).
* If the answer is **No**, use **Composition**. (e.g., An Order and its OrderLineItems—if the order is deleted, the individual line items have no meaning).
...
Imagine you are building a graphic design application. You know that every shape in your system will have a **Color** and a way to **Calculate Area**. However, the math for calculating the area of a generic "shape" doesn't exist—it depends on what the shape actually is.
...
* **Motivation:** A real-world scenario or "problem story" that illustrates the design challenge and how the pattern’s structure solves it.
...
* **Known Uses:** Examples of the pattern found in real-world systems or frameworks (e.g., how the *Decorator* pattern is used in Java’s I/O streams).
